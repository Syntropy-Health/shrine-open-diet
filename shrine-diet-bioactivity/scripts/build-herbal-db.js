...
// CSV parsing helpers
// ---------------------------------------------------------------------------
function readCsvFile(filePath, encoding = 'latin1') {
...
// ---------------------------------------------------------------------------
function loadHerbCompounds(db, partsMap) {
...
function loadBioactivities(db) {
    ...
    const activitiesByCompound = new Map();
    for (const row of rows) {
        const chem = row['CHEM']?.trim();
        const activity = row['ACTIVITY']?.trim();
        if (chem && activity) {
            const id = normalizeCompoundName(chem);
            const existing = activitiesByCompound.get(id) || new Set();
...
// ---------------------------------------------------------------------------
function backfillCompoundClass(db) {
...
async function loadFoodbContent(db, contentCsvPath, foodbCompoundMap, foodMap) {
  ...
  `);
    ...
    for await (const line of rl) {
        ...
        batch.push(() => {
            insertContent.run(compoundId, food.name, food.nameScientific, food.group, origContent, origMin, origMax, origUnit, foodPart, citation, food.publicId, compound.publicId);
        });
        contentInserted++;
...
// ---------------------------------------------------------------------------
async function loadFoodbData(db) {
    ...
    const foodbCompoundMap = new Map();
    const nameMapStmt = db.prepare(`
    INSERT OR IGNORE INTO compound_name_map (normalized_name, source, original_name, compound_id)
    VALUES (?, 'foodb', ?, ?)
  `);
    const insertNameMaps = db.transaction((data) => {
        for (const row of data) {
            const id = row['id']?.trim();
...
async function main() {
    ...
    try {
        ...
        // Summary
        const herbCount = db.prepare('SELECT COUNT(*) as cnt FROM herbs').get().cnt;
        const compoundCount = db.prepare('SELECT COUNT(*) as cnt FROM compounds').get().cnt;
        const herbCompoundCount = db.prepare('SELECT COUNT(*) as cnt FROM herb_compounds').get().cnt;
        const compoundFoodCount = db.prepare('SELECT COUNT(*) as cnt FROM compound_foods').get().cnt;
